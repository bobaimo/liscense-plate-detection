...
class visual_llm:
    ...
    def prompt_detection(self,image,prompt):
        ...
        if answer.lower().strip() in ['yes', 'yes.']:
            return True
        elif answer.lower().strip() in ['no','no.']:
            return False
        else:
...
if __name__=="__main__":
    ...
    folder_path = "cropped_box/carpark_1"  # Replace with your folder path
    for filename in os.listdir(folder_path):
        if filename.endswith(('.jpg', '.png', '.jpeg')):  # Check for image files
            ...
            if img is not None:
                ...
                print(vlm.prompt_detection(img,"What is the license plate number of the car? Give the license plate number only."))
